...
- Route thru **LAST RITES** — the red flood interstitial: whole screen red, red orb, and the truth in big letters: "This is this identity's LAST device. Removing it ends the identity — the name dies with it, nobody (including you) can ever claim it again, and every conversation key is gone forever." Interstitial rules: only the re-labelled button proceeds ("End it — forever"), anything else cancels.
- **Name release is a separate, opt-in decision** (future, needs worker support): a checkbox on LastRites — "release the name for anyone to re-claim". Unchecked (default): today's behaviour, the chain stays as a tombstone and the name is burned. Checked: the worker accepts a fresh genesis over a zero-member chain (or deletes the slot on empty fold). Safe within the trust model — friends pin party IDS, not names, so a re-claimed name mints a different identity and renders as a stranger to everyone who knew the dead one. Not built in v1; the checkbox ships greyed with "coming" or ships absent.
