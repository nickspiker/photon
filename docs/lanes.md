...
**The heavy weave (stage 4, 2026-09-08).** A full CLUTCH can now ratchet an era forward instead of opening a new lineage: `from_clutch_woven` computes the birth root and history key exactly as a fresh ceremony does, then folds them thru `derive_era_keys` with the prior era's root and history key (transcript = the ceremony id), so the result is era N+1 of the same lineage — sibling replication orders it by index, and a departed device holding the whole old era plus the ceremony transcript still cannot derive it without the fresh eggs.
The offer carries an ERA-PRIOR CLAIM (`prior_tag`, `prior_idx`, additive fields beside the keys, folded into the offer provenance so a claimed round is its own round). A peer that holds the claimed era echoes the claim and both completions weave; a peer that does not hold it does not answer (its retired era ⇒ the claimant is behind; unknown ⇒ `StaleEraObserved` asks the fleet). Unclaimed offers behave as before.
FLEET SHRINK is the trigger (decision 1): a device leaving the fold or being revoked arms `era_weave_due` on every consented friendship at the epoch AFTER the rotation the shrink starts, persisted; the keygen pickup waits for the cached fleet epoch to reach it — `chain_sync` still opens k−1, so a woven era pushed under the old epoch would still reach the leaver — then resets the round with the claim set. A `HeavyWeave` repair verdict arms one friendship immediately. Owner only; one keygen at a time.

Not yet (later stages): the consent-gated fresh channel (`ConsentFresh`, logs); the remaining ad-hoc triggers routed through `friendship_repair`; KEM decapsulation off the UI thread (inline today — a few milliseconds once per 256 rows).
