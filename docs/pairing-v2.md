...
- **BLE** (MVP wired 2026-07-14): announce beacons deliver candidates room-wide. The old device's AddDevice screen lists every registry candidate by keyed name, marking the ones whose announce beacon it currently hears "· nearby". The human matches the name their NEW device is showing ("this device: quietFalcon") to a row and taps it — that tap is the selector (in place of typing the 23 words). Consent still comes from the binding-request registry (the beacon's 12-byte proof MAC is too small for a full signature), so a tap binds using that device's signed request. Because a name-pick is not a typed-key match, the BLE/tap path uses the TWO-PHASE gate: bind (keyless) → new device folds + goes green → old device asks "did it turn green?" → confirm → rotate. Wrong pick → your device stays red → don't confirm → keyless entry, no key leaked. The lock word + `word_mac` proof from the earlier draft (single-valid-proof abort) remains available as an OPTIONAL hardening on top of name-match, not required for the MVP. The milestone-A shadow beacon is this transport's radio path (Android advertises + scans; Linux scans via bluer; desktop advertise still pending).
