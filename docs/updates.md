...
- The signed version manifest format (one VSF section per artefact) + the publish steps (deploy.sh, scripts/publish/dev-*.sh, pinned-stamp + flock-serialized as of 58bf7ed). Version discipline (2026-07-17): **patch 0 is the release marker and NOTHING else ever wears it** — deploy.sh ships .0 then immediately opens the dev line at .1, and dev publishes are publish-current-then-bump (ship the tree's version, then pre-bump for the next), so tuple equality alone answers "already on" and a dev build can never masquerade as the release.
