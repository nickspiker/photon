...
- **The leased device is a PROXY, never a member (Nick 2026-09-04)**: it does not count toward the guest's fleet, cannot be the device an identity lives on (identity liveness counts OWNED members only — the last-member gate ignores proxies), has no vote in fleet operations (no countersigning adds/departures, no release, no unattended, no sub-lease), and holds no durable vault — it is a viewport that STREAMS from the guest's fleet and composes THRU it (the shipped fleet-forward path: proxy composes, an owned device's chain transmits). Local residue is session-scoped under the wairua; recall + reboot leaves nothing meaningful, and the owner's re-attest wipe clears the rest.
- The lease and the airport session are therefore ONE runtime object — a proxy — differing only in grant lifetime (named-friend durable vs open ephemeral). Device classes settle into three: MEMBER (owned, counts, votes) · PROXY (grant-scoped, counts for nothing, killable) · HUSK (retired brand).
...
- **Sovereignty**: recall kills a grant, never touches the guest's fleet — a proxy was never a member, so there is nothing to depart. Their fleet sees the proxy go dark; their claims stay theirs.
