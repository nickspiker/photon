...
1. **tohu capsule crypto — DONE 2026-07-06.** `SealMode`, `seal_session`, `open_session`, `wairua` (via `ihi::spaghettify`), `set_boot_secret_override`; AEAD is `chacha20poly1305` direct (not `kete` — `kete → tohu` would cycle). 6 tests green (round-trip Local/Shared, wrong-wairua/reboot fails, MAC-tamper fails, bad-magic, mode-mismatch). Envelope is an INTERIM versioned binary — see step 6.
...
6. **Finalize the capsule format to full spec-compliant VSF.** Step 1 ships an interim versioned-binary envelope (`MAGIC ‖ mode ‖ [MAC] ‖ nonce ‖ ct+tag`) — fine for testing, but the finalized capsule must be a real VSF document (tooling / self-description / forward-compat). Fully encapsulated in `seal_session`/`open_session`, so it is a localized change with no caller impact.
