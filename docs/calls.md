...
2. **Media — an ephemeral UDP plane, deliberately NOT the ratchet.** Per-message ratcheting is wrong physics for 100 packets/second, and a call should be *more* forward-secret than messages. One non-VSF datagram in the whole system, stripped to a 6-byte clear header (2026-08-19): `[magic C7 11 | seq:4]` + XChaCha20-Poly1305 under the direction's step key. Everything else was derivable or already proven by the AEAD and got deleted: step = seq/100, direction and call identity live in the key (a wrong-direction or stale-call packet just fails to open). The recv worker checks the two magic bytes before the entire parse ladder and routes matches raw to the engine — no PT ack, no StatusUpdate. Code: `call/packet.rs`, `call/engine.rs`, the fast path in `network/status.rs`.
...
- **RaptorQ** over a 2-frame (20ms) window whose byte-size is TIER-SIZED (low rungs are genuinely cheap on the wire — no padding to the top rung's size). The symbol is exactly the window (windows are 8-aligned so raptorq's alignment changes nothing): 1 source + 1 repair = 2 packets per window, sealed payload `[window_id:4 | ctrl:1 = esi<<2|tier | symbol]`, and the window survives EITHER packet lost (p² loss odds — better than the old 3+2 spread AND ~3× fewer bytes). The receiver derives each window's slot walk + FEC geometry from the ctrl byte's tier bits, so mid-call rung switches decode seamlessly. The RX shape check (payload length must be exactly that rung's window) is load-bearing: raptorq panics on mis-sized symbols. Wire totals per direction: ~66 kbps floor / ~290 kbps top (vs ~218/~364 before the strip).
