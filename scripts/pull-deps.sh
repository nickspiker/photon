...
# SELF-UPDATE SAFETY: this script fast-forwards photon, and photon contains THIS FILE. Bash reads a script incrementally by byte offset, so a file that changes mid-run can resume parsing at a stale offset inside new bytes — arbitrary breakage in the middle of a deploy gate. Wrapping the whole body in a function and calling it on the final line makes bash parse everything into memory BEFORE any git command runs, so rewriting the file underneath is harmless. It is also why you do not need to `git pull` before running this: it updates photon itself, correctly, on the first invocation.
main() {
