...
#
# This does NOT build — run ./scripts/android/build.sh (release APK) first, or let deploy.sh produce it. (To build AND push in one step, use ./scripts/android/{dev,release}-{adb,network}.sh instead.) Then push the prebuilt APK to the phone with this, no rebuild.
