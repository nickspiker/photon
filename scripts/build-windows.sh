...
# then leave the signed .exe in-tree for you to hand-copy to a Windows box (it can't run on this host).
#
# Three Windows paths, don't confuse them:
#   scripts/build-windows.sh        (this) build + sign locally; copy the .exe over yourself.
#   scripts/publish/dev-windows.sh  build + sign + UPLOAD to the R2 dev channel + inject the binary's
#                                   SHA256 into the PowerShell installer.
#   deploy.sh                       full release: every target, signed, uploaded, version-bumped.
#
# The Ed25519 signature appended to the .exe is the trust check — the binary self-verifies it at every
# launch (the "SIGNATURE CHECK PASSED" line). The .sha256 sidecar the signer also emits is ONLY for the
# installer path (Windows Defender blocks running a fresh download, so the installer verifies the hash
# BEFORE execution); for a hand-copied dev .exe it's unused — the in-binary self-verify covers integrity.
#
