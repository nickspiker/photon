# Sourced, not executed. The merge-back guard: surface git worktrees that hold work not on main, so an agent's isolated worktree can't silently rot for months, get forgotten, and get redone. This is a WARNING, never a build failure — it returns 0 always. Losing work is the failure; a noisy warning is the whole point.
#
# Why this exists: worktree isolation stops parallel agents clobbering each other's files, but it does NOTHING to merge the work back. An agent finishes (or its session dies), its worktree holds good work, nothing ever merges it, `git status` on main reads clean, and you assume it's done. Isolation defers file contention from write-time to merge-time — and abandoned worktrees rot at merge-time instead of forcing a resolution. This check makes "silently orphaned" into "you're told about it now."
#
...
# The main worktree itself (the one whose HEAD is checked out here) is never flagged for being dirty — your own in-progress edits are expected. Only OTHER worktrees are.
...
# Flag one worktree. Returns 0 (success = "flagged") if it warrants a warning, 1 otherwise — so the caller can OR the result. Skips the main working tree's own dirty state (expected in-progress edits).
_worktree_check_one() {
