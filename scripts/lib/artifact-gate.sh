...
artifact_gate() {
    ...
    off=$(grep -rn --include="*.rs" -E "fs::write\(|File::create\(" src/ 2>/dev/null \
        | grep -v -E "^src/(storage/mod\.rs|lib\.rs|wave/spool\.rs|platform/|bin/|network/updates\.rs|ui/photon_app/(attachments|input)\.rs)" \
        | grep -v -E "^[^:]*:[0-9]+:\s*//" \
