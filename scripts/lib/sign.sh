...
sign_binary() {
    ...
    if [ -n "$target" ]; then
        ...
        # with no target, so every native Mac dev build skipped Apple signing and fell thru to an ad-hoc signature whose identifier is derived from the binary CONTENT (observed:
