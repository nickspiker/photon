...
pub enum StatusUpdate {
    ...
    /// One sealed chunk of a chunked blob; verified against the manifest's chunk hash off-thread, stored at its own address.
    AttachChunkReceived {
        conversation_token: [u8; 32],
        content_hash: [u8; 32],
        index: u32,
    ...
    AttachReqReceived {
        conversation_token: [u8; 32],
        content_hash: [u8; 32],
        /// Resume bitmap (bit i = chunk i wanted); None = everything.
        want: Option<Vec<u8>>,
        sender_pubkey: DevicePubkey,
        sender_addr: SocketAddr,
    },
                ...
                Ok((len, src_addr)) => {
                        ...
                        if let Some(data) = PTData::from_bytes(msg_bytes) {
                            ...
                            if let Some(complete) = complete_bytes {
                                ...
                                if let Some(data) = received_data {
                                    ...
                                    // Chunked attachment: manifest, then chunks (typed attachments Phase 1).
                                    else if let Ok(((conversation_token, content_hash, sealed), sender_pubkey)) =
                                        crate::network::fgtw::protocol::parse_attach_manifest_vsf(&data)
                                    {
                                        if !is_known_sender_pt(&sender_pubkey) {
                                            crate::log("PT: attach_manifest REJECTED - unknown sender");
                                            continue;
                                        }
                                        send_status_update(
                                            &status_tx_recv,
                                            StatusUpdate::AttachManifestReceived {
                                                conversation_token,
                                                content_hash,
                                                sealed,
                                    ...
                                    {
                                        if !is_known_sender_pt(&sender_pubkey) {
                                            crate::log("PT: attach_chunk REJECTED - unknown sender");
                                            continue;
                                        }
                                        send_status_update(
                                            &status_tx_recv,
                                            StatusUpdate::AttachChunkReceived {
                                                conversation_token,
                                                content_hash,
                                                index,
                                                sealed,
                                    ...
                                    // Attachment blob request (tap on a pill whose blob hasn't arrived)
                                        ...
                                        (conversation_token, content_hash, want),
                                        sender_pubkey,
                        ...
                        && msg_bytes[3] == b'<'
                    {
                        ...
                        // Trust is still applied downstream: the app's CLUTCH handler gates on fold-respecting knows_device.
                        {
                            ...
                            if let Ok(((conversation_token, content_hash, sealed), sender_pubkey)) =
                                crate::network::fgtw::protocol::parse_attach_manifest_vsf(msg_bytes)
                            {
                                {
                                    let ack_bytes = {
                                        let pt_mgr = pt_recv.lock().unwrap();
                            ...
                            if let Ok(((conversation_token, content_hash, index, sealed), sender_pubkey)) =
                                crate::network::fgtw::protocol::parse_attach_chunk_vsf(msg_bytes)
                            {
                                {
                                    let ack_bytes = {
                                        let pt_mgr = pt_recv.lock().unwrap();
                            ...
                            if let Ok(((conversation_token, content_hash, want), sender_pubkey)) =
                        ...
                        Ok(message) => {
                            ...
                            match message {
                                FgtwMessage::StatusPing {
                                    ...
                                    // PING REFLECTION — the probe arm's asymmetry killer, on the steadier signal. A DIRECT ping from a known device proves src_addr is a working return path (their NAT opened it toward us), and for a relay-only pair it is the ONLY direct frame that ever arrives: the side with the validated path keeps it warm with pings, so the probe arm's reflection never gets a trigger (field 2026-08-13: Emma direct→Nick while both Nick devices held only her unreachable LAN row and pinged relay forever — both fleets publish self-claimed :4383 records no NAT honours). Probe back at the proven source; the ack validates OUR direction, and its observed-addr echo teaches us the true public mapping our next announce publishes. Relay-injected pings carry the sentinel address, which is_bogus_addr rejects.
