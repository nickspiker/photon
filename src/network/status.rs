...
pub struct HistorySendRequest {
    ...
    /// Devices to ALSO send the whole frame to over the relay pipe (same rule as MessageRequest::relay_to: filled when no validated direct path, or when answering a request that itself arrived over the relay). PT's own relay fallback needs ~31s of failed retries to engage — longer than the requester's expiry — so a relay-only pair starved forever waiting on a ladder that never completed (peer_m↔peer_b history recovery, 2026-07-24).
    pub relay_to: Vec<[u8; 32]>,
}
