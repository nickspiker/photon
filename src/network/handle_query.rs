...
use crate::network::fgtw::{bootstrap::load_bootstrap_peers, FgtwTransport, PeerRecord};
...
pub struct HandleQuery {
    ...
    // UDP socket for P2P and StatusChecker
    // Starts with random port, upgraded to preferred port on first query if available
...
impl HandleQuery {
    ...
    fn new_internal(
        ...
        // Bind UDP socket immediately with random port (upgraded on first query if preferred available)
        let initial_socket = UdpSocket::bind("[::]:0").expect("Failed to bind UDP socket");
        let initial_port = initial_socket
            .local_addr()
            .expect("Failed to get socket address")
            .port();
        crate::log_info(&format!(
            "Network: Initial bind on UDP port {}",
            initial_port
        ));
    ...
    fn spawn_query_worker(
        ...
        thread::spawn(move || {
            ...
            while let Ok(handle) = rx.recv() {
                ...
                // On first query, try to upgrade to preferred port
                if first_query {
                    first_query = false;
                    let preferred_port = photon_port(&handle);
                    let current_port = *port.lock().unwrap();

                    if current_port != preferred_port {
                        if let Ok(new_socket) = UdpSocket::bind(format!("[::]:{}", preferred_port))
                        {
                            crate::log_info(&format!(
                                "Network: Upgraded to preferred port {} (from {})",
                                preferred_port, current_port
                            ));
                            *socket.lock().unwrap() = Arc::new(new_socket);
                            *port.lock().unwrap() = preferred_port;
                        } else {
                            crate::log_info(&format!(
                                "Network: Preferred port {} busy, keeping {}",
                                preferred_port, current_port
                            ));
                        }
                    }
                ...
                // Get current port for FGTW query
                let current_port = *port.lock().unwrap();
            ...
            while let Ok(handle) = rx.recv() {
                ...
                let result = if let Some(peer) = peers.first() {
                    ...
                    let bootstrap_result = tokio::runtime::Builder::new_current_thread()
                        ...
                        .build()
                        .expect("Failed to create tokio runtime")
                        .block_on(load_bootstrap_peers(&keypair, handle_proof, 41641, &handle));
