...
//
// crypto/ ├── blind.rs ── friend-blinded private identity secret S (RAM-only, never persisted) │   enum PrivateS { None, Provisional, Live { s, s_id } } ::secret(), live() │   derive_blind_pad(device_secret, friend_seed) ── per-(device,friend) OTP pad │   make_blind_blob/open_blind_blob ── (S⊕pad)‖check, 64B, fail-closed │   s_check(s), s_id(s) ── tamper commitment + 4-byte tag epoch │   seal_sibling_s/open_sibling_s(s, chain_key) ── kete AEAD S-transfer to a fleet sibling (never rides chat: chain messages persist) │ ├── chain.rs ── the braid: rolling-chain encryption (512-link, 16KB; see BRAID.md) │   struct Chain { links: [[u8;32]; 512], last_ack_time: Option<EagleTime> } │     ::from_bytes(), to_bytes(), from_full_bytes() │     ::current_key(), link(idx), links() │     ::advance(eagle_time, our_plaintext, their_plaintexts) — the braid: weaves up to two prior peer plaintexts (length-prefixed, peer-entropy-first) │   enum ChainError { DecryptionFailed, InvalidMessage, NotInitialized, │     EncryptionFailed(String), SignatureInvalid, InvalidEncoding, │     AckMismatch, ParticipantNotFound } │   CHAIN_LINKS=512, HISTORY_LINKS=256, ACTIVE_LINKS=256 │   LINK_SIZE=32, CHAIN_SIZE=16384, CURRENT_KEY_INDEX=511 │   L1_SIZE=30720, L1_ROUNDS=3 │   derive_salt(prev_plaintext, chain) │   generate_scratch(chain, salt), generate_scratch_at_offset(chain, salt, off) │   generate_confirmation_smear(message, chain) │   generate_ack_proof(eagle_time, plaintext_hash, chain) │   verify_ack_proof(eagle_time, plaintext_hash, chain, received_proof) │   derive_nonce(eagle_time), encrypt_layers(), decrypt_layers() │ ├── clutch.rs ── 8-algorithm parallel key ceremony │   smear_hash(data), derive_conversation_token(participant_seeds) │   derive_ceremony_instance(offers), spaghettify(input) │   sibling_party_id(device_pubkey) ── fleet-weave party id (device-derived; sibling ceremonies key on this instead of the shared handle_hash) │ ├── handle_proof.rs ── memory-hard handle attestation (~1s) │   handle_proof(hash) │ ├── keys.rs ── identity key management (TODO) │ ├── self_verify.rs ── Ed25519 binary signature verification │   AUTHOR_PUBKEY, SYSTEM_PUBKEYS │   is_system_pubkey(pubkey), verify_binary_hash() │ └── shards.rs ── social recovery key sharding (TODO)
//
// network/ ├── fgtw/ ── Fractal Gradient Trust Web (Kademlia DHT) │   ├── blob.rs ── binary blob storage/retrieval │   │ │   ├── bootstrap.rs ── initial peer discovery │   │   load_bootstrap_peers() │   │ │   ├── fingerprint.rs ── device fingerprint derivation: derive_device_keypair/get_machine_fingerprint/FgtwPaths (oracle read stays in photon); Keypair itself now lives in the fgtw crate (fgtw::keys::Keypair), re-exported so crate::network::fgtw::Keypair is unchanged │   │ │   ├── node.rs ── Kademlia routing table, k-buckets │   │ │   ├── peer_store.rs ── peer caching │   │   struct PeerStore │   │ │   ├── protocol.rs ── VSF-encoded FGTW + CLUTCH messages │   │   enum FgtwMessage { ... + PhonebookRequest/Response (peers-are-FGTW gossip) + standalone hist_req/hist_page frames (build/parse_history_request_vsf, build/parse_history_page_vsf, HistoryRequestPayload — friend-history recovery, canonical sign_file + read_verified) + blind frames (build/parse_blind_{put,ack,get,srv}_vsf, parse_any_blind_frame, BlindFrameKind/BlindFramePayload — friend-blinded S deposits/serves; ack sent only after the friend's disk commit) + AvatarRequest/Response (av_req/av_resp — direct P2P avatar exchange between mutual contacts, signed over the avatar bytes' hash) + Reflect/ReflectResponse (reflect/reflect_resp — open-tier STUN address reflection, answerable by any directory-serving node) + StatusPong now carries observed_addr (peer-echoed reflexive) } │   │   struct PeerRecord { handle_proof, device_pubkey, ip, local_ip, last_seen, signature } (self-signed: ::sign/::verify via the Ed25519 device_pubkey, so gossiped records are relay-independent), SyncRecord │   │ │   ├── fleet.rs ── FGTW CLIENT layer; all the pure logic now lives in the fgtw crate (shared verbatim by every app + the worker), re-exported here so call sites are unchanged. Crate modules: fgtw::fleet (chain: FleetOp/MembershipBlob::{genesis,add,remove,fold,extends,is_member,to/from_vsf_bytes}, fold() IS the authorisation rule = valid sig + hash chain + signer-was-a-prior-member); fgtw::fanout (fan-out crypto: new_fleet_key/fanout_seal/open/to+from_bytes, FanoutWrap); fgtw::fstate (roster codec: RosterEntry/roster_to+from_bytes/merge_rosters); fgtw::pair (pairing words: pair_words/new_pairing_id/words_to_pair_pubkey/first_bad_pair_word/parse_pair_event, PairRequest, *_signing_bytes). the fetch-then-sign oracle is fgtw::client (fetch/publish/ensure_member/bind/unbind/current_members, fan-out post/fetch/rotate/recover, roster push/pull, pairing post/fetch/poll) — transport-agnostic via FgtwTransport + FleetSealer traits. fleet.rs is now just photon's BINDING: struct PhotonTransport (HTTP over crate::network::http's pooled reqwest + short errors) + struct PhotonSealer (roster AEAD over kete) + same-signature wrapper fns that inject them (call sites unchanged) │   └── relay.rs ── relay node logic │ ├── clock_check.rs ── one-shot wall-clock sanity check via nunc-time consensus (desktop-only; warn-only — never corrects the clock) │   spawn_clock_check(tx, wake) ── off-thread nunc::query, posts ClockCheckResult │   struct ClockJumpDetector ── monotonic-vs-wall jump trigger for mid-session re-checks │   enum ClockCheckResult { Ok { offset_secs, confidence_secs, sources_used/queried }, Unavailable } │ ├── handle_query.rs ── handle attestation and lookup │   struct HandleQuery │     ::new(keypair, event_proxy), query(handle), query_resume(session), try_recv() │     ::try_recv_online(), search(handle), try_recv_search() │     ::set_handle_proof(proof), get_handle_proof() │     ::set_transport(), get_transport(), port(), socket() │   enum QueryRequest { FirstAttest(String), Resume(tohu::SessionIdentity) } │   enum QueryResult { Success(AttestationData), AlreadyAttested, Error } │   struct AttestationData { handle_proof, identity_seed, │     contacts, friendships, avatar_pixels, peers } │ ├── history_pages.rs ── KEY-AGNOSTIC history-backfill page codec (phase-2/fleet reuses it verbatim) │   seal/open_history_page(page, key) ── VSF section + kete ChaCha20-Poly1305; metadata (oldest_osc, more) inside the seal │   struct HistoryRow { timestamp, content, sender_outgoing, delivered }, HistoryPagePlain { rows, oldest_osc, more } │   MAX_PAGE_ROWS=50, MAX_PAGE_BYTES=24KB │ ├── http.rs ── shared pooled HTTP for FGTW │   runtime() (one persistent tokio) · async_client() · blocking() — reused reqwest clients keep TLS warm │ ├── inspect.rs ── network diagnostic utilities, VSF disk I/O │   vsf_write(path, encrypted, label, decrypted, device_secret) │   vsf_read(path, label, device_secret) │ ├── peer_updates.rs ── peer state change notifications │   struct PeerUpdate, PeerUpdateClient │ ├── pt/ ── Photon Transfer (large message transport) │   ├── buffer.rs ── reassembly buffer │   ├── packets.rs ── packet framing │   │   struct PTSpec │   ├── state.rs ── transfer state machine │   │   enum Direction, TransferState │   │   enum PTError, struct OutboundTransfer │   └── window.rs ── sliding window flow control │   struct PTManager { outbound, inbound, keypair, next_stream_id } │     SINGLE_PACKET_MAX=1024 │     ::new(keypair), send(addr, data), send_with_pubkey() │     ::handle_spec(), handle_spec_ack(), handle_data(), handle_ack() │   struct RelayInfo { recipient_pubkey, payload } │   struct TickSend { peer_addr, wire_bytes, tcp_payload, relay } │ ├── status.rs ── P2P ping/pong, CLUTCH orchestration │   struct StatusChecker │     ::new(socket, keypair, contacts, sync_records, event_proxy) │   enum StatusUpdate { │     Online { peer_pubkey, is_online, peer_addr, sync_records }, │     ChatMessage { conversation_token, prev_msg_hp, ciphertext, ... }, │     MessageAck { conversation_token, acked_eagle_time, plaintext_hash }, │     PTReceived { peer_addr, data }, │     PTSendComplete { peer_addr }, │     ClutchOfferReceived { conversation_token, offer_provenance, ... }, │     ClutchKemResponseReceived { ... }, │     ClutchCompleteReceived { ... }, │     AvatarRequestReceived { sender_pubkey, sender_addr }, AvatarReceived { responder_pubkey, avatar_vsf, sender_addr } (P2P avatar exchange; UI gates both on ClutchState::Complete = mutual), │     HistoryRequestReceived { conversation_token, before_osc, limit, request_id, sent_osc, ... }, HistoryPageReceived { conversation_token, request_id, sealed, ... } (friend-history backfill; UI authorizes + serves/merges), │     BlindFrameReceived { kind, conversation_token, request_id, blob, found, sent_osc, ... } (blind_put/ack/get/srv — the S machinery; rides the hist send channel), │     LanPeerDiscovered { handle_proof, local_ip, port }, │     ReflexiveLearned { addr } (our own public addr from peer-echoed reflection) } │   struct PingRequest { peer_addr, peer_pubkey } │   struct MessageRequest { peer_addr, recipient_pubkey, conversation_token, │     prev_msg_hp, ciphertext, eagle_time } │   struct AckRequest { peer_addr, recipient_pubkey, conversation_token, │     acked_eagle_time, plaintext_hash } │   struct PTSendRequest { peer_addr, data } │   struct HistorySendRequest { peer_addr, alt_addr, recipient_pubkey, vsf_bytes } (pre-built signed hist_req/hist_page; ::send_history) │   struct ClutchOfferRequest, ClutchKemResponseRequest, │     ClutchCompleteRequest, LanBroadcastRequest, ClearPtSendsRequest │ ├── tcp.rs ── TCP fallback for large payloads │   send(stream, data), recv(stream) │ ├── traverse/ ── NAT traversal (P0+: reflexive discovery; candidate model, hole-punch, directory tier to follow) │   └── reflexive.rs ── peer-echoed reflexive/STUN: struct ReflexiveState (own public addr, quorum-adopted from pong observed_addr + ReflectResponse) │ └── udp.rs ── UDP socket utilities async send(socket, data, addr), send_sync(socket, data, addr) canon_socketaddr(addr) (::ffff:→v4) log_received(data, addr) get_local_ip(), get_broadcast_addr()
//
// platform/ ├── mod.rs ── platform detection └── jni_android.rs ── Android JNI bridge
//
// storage/ ── the flat vault layer lives in the kete crate (FlatStorage, re-exported here); conversation content (rows/blobs) in the rarangi crate. Vault entries are addressed by a flat 32-byte key, never a path: vault_key(domain, scope) = blake3_kdf("photon.storage.entry.v0", domain || scope), where domain is a plain word ("avatar", "state", "chains", ...) and scope is the 32-byte identity the entry is about (our vault_seed for self/global, a peer seed for per-peer, a friendship_id for per-conversation). No hex, no base64, no tree paths anywhere except the one vault-root .vsf filename. │ ├── mod.rs ── kete re-exports (FlatStorage, StorageError, encrypt/decrypt_bytes, App, APP) + raw-file helpers │   vault_key(domain, scope) ── canonical 32-byte vault address for an entry │   write_file/read_file(path, label) ── raw atomic file I/O (now only inspect.rs diagnostics use it; avatars moved into the vault) │   photon_config_dir() │ ├── cloud.rs ── FGTW cloud backup (contacts sync) │   struct CloudContact, enum CloudError │   contacts_storage_key(identity_seed, device_secret) ── FGTW network locator (base64url, on the wire only) │   contacts_encryption_key(identity_seed, device_secret) │ ├── contacts.rs ── contact + conversation storage via FlatStorage (byte-addressed) │   struct ContactIdentity │   derive_identity_seed(handle) │   save/load_contact_list(storage) ── index at vault_key("contacts", storage.vault_seed()) │   save/load_contact_state, save/load_all_contacts(storage) ── per-peer at vault_key(domain, their_seed); state is keyed by contact.handle_hash (= the party id — identity seed for friends, sibling pid for fleet siblings) │   save/load_sibling_list, load_all_siblings(our_handle, our_hp, storage), delete_sibling(device_pubkey, storage) ── fleet-sibling index at vault_key("siblings", vault_seed); siblings never enter the contacts index (handle-string dedup would collapse them) │   save/load_messages(contact, storage) ── conversation CONTENT, now rarangi rows: table = friendship_id bytes (derived early from sorted participant seeds; self/1:1/group/fleet), pk = the message's eagle_time as u64 (monotonic clock → key order == chronological; doubles as the braid's weave reference); row also stores content_hash + ack_hash (re-ACK lost-ACK heal) + recovered (friend-attested provenance); load_messages sorts keys numerically (recovery inserts older rows later) │   save_messages_page(their_seed, msgs, storage) ── O(page) row writes for backfill merges │   load_message_page_before(their_seed, before_osc, max_rows, max_bytes, storage) ── newest-first history page serve (rows ascending + more flag) │   contact_state also persists hist_oldest/hist_complete (the backfill cursor — survives restarts + re-keys) │   save/load/delete_clutch_keypairs + save/load/delete_clutch_slots ── MEMORY-ONLY NO-OPS: ephemeral ceremony scratch (McEliece-heavy) is never persisted; contact.clutch_slots / clutch_our_keypairs are the sole truth, a mid-ceremony restart re-runs the off-thread keygen. Durably persisting them grew the dual-mirror vault and the fallocate/zero grow froze the UI mid-ceremony │ ├── friendship.rs ── per-friendship chain STATE (the ratchet machinery, not content) via FlatStorage at vault_key("chains", friendship_id); v6 adds the optional history_key (spaghettify-derived at ceremony birth in FriendshipChains::from_clutch — seals history pages OUTSIDE the ratchet; zeroized on re-key supersede) save/load_friendship_chains(chains/id, storage) load_all_friendships(friendship_ids, storage) delete_friendship_chains(friendship_id, storage) │ └── settings.rs ── user-adjustable app settings, plain VSF at photon_config_dir()/settings.vsf (non-secret, NOT the vault) │   struct Settings { hex_head, hex_tail } ── log hex-elision lengths │     ::load_or_create() (self-creates with defaults), ::apply() (pushes to vsf::inspect::set_hex_elision unless VSF_HEX_HEAD/TAIL env override) NB: kete::FlatStorage gained byte-addressed write_addr/read_addr/delete_addr(&[u8;32]) — used by all photon storage above — alongside the string write/read/delete still used by rarangi (whose table/pk strings kete hashes to addresses internally).
//
// types/ ├── contact.rs ── contact/friendship re-exports │   struct PartySlot { handle_hash, offer, kem_secrets_from/to_them, ... } │     ::new(handle_hash), is_complete() │   struct ChatMessage { content, timestamp, is_outgoing, delivered, ack_hash, recovered } (ack_hash = plaintext_hash we ACK a received msg with, persisted so a duplicate retransmit re-ACKs — lost-ACK heal; recovered = friend-attested row from history recovery) │   struct HistoryRecovery { oldest_recovered_osc, complete, in_flight, next_request_osc, urgent, was_complete_before } (newest-first backfill cursor; durable bits persist as hist_oldest/hist_complete in contact state) │     ::new(content, is_outgoing), new_with_timestamp() │   struct HandleText(String) ::new(s), as_str() │   struct ContactId([u8;32]) ::from_pubkey(), from_bytes(), as_bytes() │   enum ClutchState { Pending, AwaitingProof, Complete } │   const CHAIN_PROBE_MARKER (reserved sentinel content for the hidden chain-weave probe message — a normal chat msg that rides the ratchet with its UI suppressed to prove the chain end-to-end after CLUTCH Complete) │   enum TrustLevel { Stranger, Known, Trusted, Inner } │   struct Contact { │     id, handle, handle_proof, handle_hash, public_identity, │     ip, local_ip, local_port, │     relationship_seed, friendship_id, clutch_state, │     clutch_our_keypairs, clutch_slots, ceremony_id, │     clutch_pending_kem, clutch_offer_sent, clutch_*_proof, │     clutch_*_in_progress, completed_their_hqc_prefix, │     offer_provenances, trust_level, added, last_seen, │     is_online, messages, message_scroll_offset, │     avatar_pixels, avatar_scaled, avatar_scaled_diameter, │     chain_woven/probe_sent/their_probe_seen/chain_advanced_by_ack (chain-weave probe: runtime-only, gates "weaving the chain"→"secured" and cancels ceremony rebroadcast once the ratchet is proven both ways), │     is_sibling (fleet weave: this "contact" is one of OUR OWN devices — party id in handle_hash is sibling_party_id(device), hidden from UI/roster, persists under the sibling index), │     deposited_blinds/blind_deposited/blind_in_flight/blind_probe_missed (friend-blinded S: their deposits with us + our confirmed-deposit flag + runtime op state) } │     ::new(), new_sibling(our_handle, our_hp, sibling_device), with_ip(), with_seed(), with_trust_level() │     ::update_last_seen(), can_be_custodian() │     ::get_ceremony_id(), init_clutch_slots() │     ::get_slot_index(), get_slot_mut(), get_slot() │     ::all_slots_complete(), insert_message_sorted(), clutch_status_detail() │ ├── device.rs ── device identity │   struct DevicePubkey │   ed25519_secret_to_x25519(ed_secret) │ ├── friendship.rs ── friendship and ceremony types │   struct CeremonyId([u8;32]) │     ::derive_base(handle_hashes), derive(handle_hashes, provenances) │     ::from_bytes(), as_bytes() │   struct FriendshipId([u8;32]) │     ::derive(handle_hashes), from_bytes(), as_bytes() │     ::to_base64(), from_base64() │   struct FriendshipChains { friendship_id, conversation_token, │     chains, participants } │ ├── handle.rs ── handle type │   struct Handle { text, key } │     ::new(), to_handle_proof(), username_to_handle_proof() │ ├── peer.rs ── peer connection state │   struct Peer { public_identity, address, last_seen, connection_state } │     ::new(), update_connection_state(), is_online() │   enum ConnectionState { Disconnected, Connecting, Connected, Authenticated } │   struct DhtAnnouncement { public_key, port, timestamp, signature } │ ├── seed.rs ── cryptographic seed │   struct Seed([u8;32]) │ └── shard.rs ── key shard structures struct KeyShard, ShardId([u8;16]), DecryptedShard struct RecoveryRequest, RecoveryApproval, ShardDistribution
//
// ui/ ├── app.rs ── application state machine │   struct PhotonApp (main app state — see text_editing.rs for text methods) │   struct TextState { chars, widths, width, blinkey_index, │     scroll_offset, selection_anchor, textbox_focused, is_empty } │     ::new(), insert(), insert_str(), remove(), delete_range() │   struct TextLayout { center_x/y, box_width/height, usable_*, │     margin, font_size, line_height, button_area } │     ::new(w, h, span, ru, app_state) │     ::blinkey_x(text), blinkey_y(), text_start_x(text) │   struct PixelRegion { x, y, w, h } │     ::new(), from_signed(), contains(), right(), bottom(), center() │   struct Layout { logo_spectrum, photon_text, textbox, │     attest_block, contacts, header, message_area } │   struct AttestBlockLayout { error, textbox, hint, attest } │     ::new(block) │   struct ContactsHeaderLayout { avatar, handle, hint, textbox, separator } │     ::new(block), avatar_center_radius() │   struct ContactsRowsLayout { rows, row_height, avatar_diameter, ... } │     ::new(), row_region(), row_avatar_center() │     ::row_text_position(), visible_row_count() │   struct ContactsUnifiedLayout { user_avatar, handle, hint, ... } │   struct ClutchKeygenResult { contact_id, keypairs } │   struct ClutchKemEncapResult { contact_id, kem_response, local_secrets, ... } │   struct ClutchCeremonyResult { contact_id, friendship_chains, │     eggs_proof, their_handle_hash, ceremony_id, ... } │   soft_limit(x, max) │ ├── avatar.rs ── avatar encoding/upload/download │   AVATAR_SIZE, encode_avatar_from_image(image_data) │ ├── colour.rs ── colour utilities │ ├── compositing.rs ── compositing pipeline, layout calculation, rendering │ ├── display_profile.rs ── ICC colour profile conversion │   struct DisplayConverter { xyz_to_display, r/g/b_trc } │     ::new(), convert_avatar(vsf_rgb) │ ├── drawing.rs ── primitive drawing (circles, lines, fills) │ ├── keyboard.rs ── input handling (key events → app state changes) │ ├── mouse.rs ── mouse/touch input │ ├── renderer_*.rs ── platform-specific renderers (7 backends) │   android, linux_softbuffer, linux_wgpu, macos, │   macos_softbuffer, redox, windows │ ├── text_editing.rs ── text input methods on PhotonApp │   ::textbox_is_focused(), font_size(), textbox_width/height() │   ::textbox_center_y(), textbox_left/right() │   ::recalculate_char_widths(), render_text_clipped() │   ::update_text_scroll(), update_selection_scroll() │   ::get_selection_range(), delete_selection(), get_selected_text() │   ::paste_text(), handle_blinkey_left(), next_blink_wake_time() │   ::start/stop/undraw/draw/flip_blinkey() │   ::add/subtract_blinkey_top/bottom(), invert_selection() │ ├── text_rasterizing.rs ── font rendering (cosmic-text) │ ├── state.rs ── AppState { Launch, Ready, Searching, Conversation, AddDevice, Settings(SettingsPage), Connected } · SettingsPage { You, Fleet, Security, Recovery, Appearance, Notifications, Updates, Diagnostics, About } (::ALL, ::label) │ ├── settings_widgets.rs ── struct Checkbox (fluor has no toggle; labelled on/off box, ::new/::render_content_into/::take_toggle, Click+Key+Focus+Hover) │ ├── settings_layout.rs ── struct SettingsLayout { header, rail, content } ::compute(vp)/rail_rows()/content_body()/content_rows() (nav-rail vs content split via fluor Region) │ └── theme.rs ── colour palette
//
