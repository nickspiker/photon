...
impl Chain {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        // History [0..256) stays zeros Active [256..512) gets the CLUTCH-derived bytes
...
///
/// Layer 1: Build VSF section (done by caller) Layer 2: ChaCha20 encryption Layer 3: XOR with scratch pad
pub fn encrypt_layers(
    ...
    // Layer 3: XOR with scratch pad (cycling if message > scratch) — cipher-independent.
...
///
/// Layer 3: XOR with scratch pad Layer 2: ChaCha20 decryption Layer 1: Parse VSF section (done by caller)
pub fn decrypt_layers(
    ...
    // Layer 3 (reverse): XOR with scratch pad — cipher-independent.
    let mut intermediate = ciphertext.to_vec();
    for (i, byte) in intermediate.iter_mut().enumerate() {
        *byte ^= scratch[i % scratch.len()];
    }

    // Layer 2 (reverse): XChaCha20 (current format, 2026-08-18).
    let chacha_key = blake3::derive_key("photon.chain.xchacha.v1", &chain.links[key_index]);
    let nonce = derive_xnonce(eagle_time);
    let mut cipher = XChaCha20::new(&chacha_key.into(), &nonce.into());
...
mod tests {
    ...
    fn test_chain_from_bytes() {
        ...
        // Active [256..512) should have our test data First active link at [256] has bytes 0..31
