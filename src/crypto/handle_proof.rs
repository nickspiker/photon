...
pub fn handle_proof(hash: &blake3::Hash) -> blake3::Hash {
    ...
    // SAFETY: Buffer is filled by Phase 1 and Phase 2 before any reads.
    // fill_size is aligned to CHUNK_SIZE boundary, ensuring no gaps.
    unsafe {
    ...
    for round in 0..ROUNDS {
        ...
        let fill_size_raw = min_fill + ((hash_num % U256::from(fill_range as u128)).as_u128() as usize);
        ...
        while curr_start + CHUNK_SIZE <= SIZE {
            ...
            let read_idx = (prev_hash_num % U256::from(curr_start as u128)).as_u128() as usize;
