...
pub fn text(msg: Msg) -> Cow<'static, str> {
    match msg {
        ...
        Msg::EndWave => "Завершить волну".into(),
        ...
        Msg::WaveStart => "\u{260E} Волна".into(),
        ...
        Msg::IncomingWave => "\u{260E} входящая волна".into(),
        Msg::IncomingWaveNoPath => "\u{260E} входящая волна \u{2014} \u{26A0} нет прямого пути".into(),
        Msg::WaveStartNoPath => "\u{260E} no direct path".into(),
        Msg::WaveBackNoPath => "no direct path".into(),
        Msg::WaveActiveNoPath(dur) => format!("\u{260E} {dur} \u{2014} \u{26A0} нет прямого пути").into(),
        ...
        Msg::WaveReconnecting => "\u{26A0} переподключаемся\u{2026}".into(),
        Msg::WaveDroppedRow => "\u{260E} волна \u{2014} оборвалась".into(),
        Msg::WaveDroppedDur(dur) => format!("\u{260E} волна \u{2014} оборвалась \u{2014} {dur}").into(),
        Msg::WaveChipElsewhere(dev) => format!("\u{260E} волна идёт на устройстве {dev}").into(),
        Msg::WaveChipElsewhereUnknown => "\u{260E} волна идёт на другом устройстве".into(),
        Msg::StopPlayback => "\u{25A0} Стоп".into(),
        Msg::WaveEndedDur(dur) => format!("\u{260E} волна \u{2014} {dur}").into(),
        // «волна:» с двоеточием вместо «волна к …»: дательный падеж не налезает на подставленное имя.
        Msg::WavingName(name) => format!("\u{260E} волна: {name}\u{2026}").into(),
        Msg::WaveRow => "\u{260E} волна".into(),
        Msg::MissedWaveRow => "\u{260E} пропущенная волна".into(),
        Msg::WaveDeclinedRow => "\u{260E} волна отклонена".into(),
        ...
        // ---- compact wave bar ----
        Msg::WaveBarWaving(name) => format!("\u{260E} идёт волна: {name}").into(),
        Msg::WaveBarInWave(name) => format!("\u{260E} в волне \u{2014} {name}").into(),
        Msg::WaveBarKeepRecording => "\u{260E} сохранить эту запись?".into(),
        ...
        Msg::RingIncomingWave => "Звонок при входящей волне".into(),
        ...
        Msg::VibrateIncomingWave => "Вибрация при входящей волне".into(),
        ...
        Msg::WaveLiveStats { rung, freq, loss, buf } => format!("{rung} \u{00B7} {freq} \u{00B7} потери {loss} из {} \u{00B7} буфер {buf}", fmt_num(256)).into(),
        ...
        Msg::AboutWaveBeamProse => "Волна — это голос. Луч — это свет: видео, оно уже в пути. У Photon нет звонков. Звонок отдаёт твой голос третьей стороне, чтобы та его донесла, и все делают вид, что никто его не сохранил — но данные сегодня и ЕСТЬ носитель, так что любой цифровой звонок записан, просто не тобой. У волны нет оператора, нет аккаунта, нет компании посередине: твой голос идёт между твоими устройствами и больше нигде не оседает.\nКаждая волна и каждый луч записываются честно: когда всё кончается, запись ТВОЯ — хранить, удалить или переслать, как любое сообщение или письмо; ты ведь доверяешь тому, кому шлёшь волну. Запись, которая никогда не касалась ни Google, ни Meta, — это совсем не то же самое, что запись, живущая на их серверах.".into(),
