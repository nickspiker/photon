...
pub fn text(msg: Msg) -> Cow<'static, str> {
    match msg {
        ...
        Msg::BringIntoGroup => "Позвать в группу".into(),
        Msg::NewGroup => "Новая группа".into(),
        Msg::GroupTitlePrompt => "Название группы".into(),
        Msg::FoundGroupPill => "Основать".into(),
        Msg::HistoryFromGenesis => "Новички видят всё".into(),
        Msg::HistoryFromJoin => "Новички видят с момента входа".into(),
        Msg::GroupAlone => "только ты".into(),
        Msg::JoiningStatus => "вступаю\u{2026}".into(),
        Msg::CatchingUpStatus => "догоняю\u{2026}".into(),
        Msg::LeftStatus => "ты вышел".into(),
        Msg::PendingMember => "Ожидание\u{2026}".into(),
        Msg::GroupComposeSoon => "Сообщения в группах появятся в следующей сборке".into(),
        Msg::GroupPageName(p) => match p { crate::ui::state::GroupPage::About => "О группе", crate::ui::state::GroupPage::Add => "Добавить", crate::ui::state::GroupPage::Manage => "Управление" }.into(),
        Msg::Members => "Участники".into(),
        Msg::MemberStanding => "в группе".into(),
        ...
        Msg::LeaveGroupPill { armed } => if armed { "Нажми ещё раз \u{2014} выйти" } else { "Выйти" }.into(),
        Msg::LeaveGroupNote => "Выход публикует твой подписанный уход; остальные чеканят новый ключ, которого у тебя не будет. Твоя копия истории остаётся здесь читаемой.".into(),
        Msg::AddToGroupNote => "Тому, кого ты добавишь, группа предлагается в вашей беседе; вступают нажатием Вступить.".into(),
        Msg::NobodyToAdd => "Все, кого ты знаешь, уже здесь.".into(),
        Msg::YouLeftNote => "Ты вышел из этой группы.".into(),
        Msg::GroupsExplainHead => "Как устроены группы".into(),
        Msg::GroupsExplainProse => "Группа — это дружба больше чем на двоих. У неё нет владельца: тот, кто её начал, — просто первый участник.\nТы входишь через друга. Участник предлагает тебе группу в беседе, которая у вас уже есть, и ты вступаешь нажатием Вступить. Никто не может засунуть тебя в группу.\nЛюбой участник может позвать того, кого знает. Никто не может удалить никого, кроме себя. Если кто-то нежелателен, остальные заводят новую группу, а старая угасает.\nНазвание группы — лишь ярлык, общий для всех, и любой может его сменить. Её настоящая личность — случайное число, отчеканенное при рождении, так что название нельзя занять, продать или украсть.\nКаждое сообщение шифруется один раз и отправляется каждому участнику. Выход публикует твой подписанный уход; остальные чеканят новый ключ, которого у тебя не будет, а твоя копия истории остаётся тебе читаемой.\nИстория до твоего входа не показывается, если группа не была основана иначе.".into(),
        Msg::DefaultHistoryHead => "Группы, которые ты создаёшь".into(),
        Msg::DefaultHistoryNote => "Что видят новички — задаётся при основании группы.".into(),
        Msg::YourGroups => "Твои группы".into(),
        Msg::NoGroupsYet => "Групп пока нет. Открой панель друга и выбери Позвать в группу.".into(),
        Msg::OfferLine { sponsor, title, n } => format!("{} зовёт тебя в {} \u{00b7} {}", sponsor, title, n).into(),
        Msg::OfferJoined { title } => format!("в группе {}", title).into(),
        Msg::OfferExpired => "приглашение истекло".into(),
        Msg::OfferJoin => "Вступить".into(),
        Msg::OfferWaiting { name, title } => format!("ты позвал {} в {} \u{00b7} ждём", name, title).into(),
        Msg::OfferAccepted { name, title } => format!("ты позвал {} в {} \u{00b7} вступил", name, title).into(),
