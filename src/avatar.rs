...
pub fn encode_avatar_from_image(image_data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    resizer
        .resize(
            linear_vsf_cropped.as_rgb(),
            linear_vsf_resized.as_rgb_mut(),
        )
...
fn encode_av1(rgb_data: &[f32], size: usize) -> Result<Vec<u8>, String> {
    ...
    for cy in 0..chroma_size {
        for cx in 0..chroma_size {
            ...
            let g = (rgb_data[idx00 + 1] + rgb_data[idx01 + 1] + rgb_data[idx10 + 1] + rgb_data[idx11 + 1]) / 4.;
            let b = (rgb_data[idx00 + 2] + rgb_data[idx01 + 2] + rgb_data[idx10 + 2] + rgb_data[idx11 + 2]) / 4.;
...
/// Get the avatars cache directory (~/.config/photon/avatars/)
pub fn avatars_dir() -> std::io::Result<std::path::PathBuf> {
    let config_dir = dirs::config_dir()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "No config dir"))?;

    let avatars_dir = config_dir.join("photon").join("avatars");
...
pub fn load_cached_avatar(handle: &str) -> Option<(usize, Vec<u8>)> {
    ...
    eprintln!("Avatar: Loading {} from local cache", handle);
...
fn save_avatar_to_cache(handle: &str, vsf_data: &[u8]) -> std::io::Result<()> {
    ...
    eprintln!("Avatar: Cached {} locally ({}...)", handle, &storage_key[..8]);
    Ok(())
}
...
pub fn load_avatar_from_bytes(vsf_data: &[u8], handle: &str) -> Option<(usize, Vec<u8>)> {
    // Verify provenance hash before processing
    if let Err(e) = vsf::verification::verify_provenance_hash(vsf_data) {
        eprintln!("Avatar: Provenance hash verification failed: {}", e);
        return None;
    }
    ...
    if parsed.encoding != b'e' {
        eprintln!("Avatar: Expected encrypted (v'e'), got: {}", parsed.encoding as char);
        return None;
    }
    ...
    let av1_data = match decrypt_av1_data(&parsed.data, handle) {
        Ok(data) => data,
        Err(e) => {
            eprintln!("Avatar: Decryption failed: {}", e);
            return None;
        }
    };
    ...
    let (width, height, pixels) = match decode_avatar(&av1_data) {
        Ok(result) => result,
        Err(e) => {
            eprintln!("Avatar: decode_avatar failed: {}", e);
            return None;
        }
    };
...
pub fn save_avatar(av1_data: &[u8], handle: &str) -> std::io::Result<()> {
    ...
    let vsf_bytes = VsfBuilder::new()
        ...
        .add_section("image", vec![
            ("pixels".to_string(), VsfType::v(b'e', encrypted)),
        ])
        .build()
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))?;
...
fn extract_av1_data(vsf_bytes: &[u8], handle: &str) -> Result<Vec<u8>, String> {
    ...
    if parsed.encoding != b'e' {
        return Err(format!("Expected encrypted (v'e'), got: {}", parsed.encoding as char));
    }
...
pub fn derive_avatar_keypair(device_secret: &SigningKey, handle: &str) -> (SigningKey, VerifyingKey) {
...
pub fn upload_avatar(device_secret: &SigningKey, handle: &str) -> Result<String, String> {
    ...
    vsf::verification::verify_provenance_hash(&local_vsf)?;
    ...
    let signed_vsf = build_signed_avatar_vsf(&av1_data, handle, &avatar_signing, &avatar_verifying)?;
    ...
    if response.status().is_success() {
        eprintln!("Avatar: Uploaded to FGTW (key: {}...)", &storage_key[..8]);
        Ok(storage_key)
    } else {
...
pub fn download_avatar(handle: &str) -> Option<(usize, Vec<u8>)> {
    ...
    let storage_key = avatar_storage_key(handle);
    let url = format!("{}/avatar/{}", FGTW_URL, storage_key);
    eprintln!("Avatar: Fetching {} from FGTW ({}...)", handle, &storage_key[..8]);

    let client = reqwest::blocking::Client::new();
    let response = client.get(&url).send().ok()?;

    if !response.status().is_success() {
        eprintln!("Avatar: FGTW returned {}", response.status());
        return None;
    }
...
pub fn scale_avatar(src: &[u8], diameter: usize) -> Option<Vec<u8>> {
    ...
    let mut resizer = resize::new(AVATAR_SIZE, AVATAR_SIZE, diameter, diameter, RGB8, Mitchell).ok()?;
    let mut dst = vec![0u8; diameter * diameter * 3];

    // Convert slices to rgb::RGB8 slices
    let src_rgb: &[rgb::RGB8] =
        unsafe { std::slice::from_raw_parts(src.as_ptr() as *const rgb::RGB8, AVATAR_SIZE * AVATAR_SIZE) };
    let dst_rgb: &mut [rgb::RGB8] =
        unsafe { std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut rgb::RGB8, diameter * diameter) };
