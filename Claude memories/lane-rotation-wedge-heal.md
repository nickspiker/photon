...
Field verdict (Nick+Mary logs, 2026-08-09): the Nick→Mary direction was fully dead — Mary at the ANCHOR of Nick's lane (lost/never-held lane state), every Nick frame gap-buffered "ahead of us" forever (unlinkable by hash AND undecryptable at position-0 keys), 2,913 retransmit lines overnight from the give-up → stall-recovery-re-arm(tip 0) → give-up loop. Nick's "ACKs take a second or two" was the retransmit spray + INLINE-AWAITED relay HTTPS legs (~1.2s each) serialising the status-thread drain: ACK queued 07:27:04.8, dispatched 07:27:14.4.
...
Open (ticketed): nonzero-STUCK-tip wedge needs per-lane head HASHES in sync records; Mary never fired the digest resync walk despite being rows behind (zero requests all session) — needs a log round post-heal.
