...
**Generalized same day into docs/key-custody.md (the voucher ladder)**: wairua (same boot) → FLEET vouches → handle on the waking device as root of last resort. REVISED same day (Nick caught the possession-only regression: a stolen backpack must never authenticate): the fleet rung moves WHERE the handle is typed (on the approver — kernel-protected input on ferros/Android; Win/Mac can't promise it), never WHETHER. Knowledge factor is non-negotiable; unattended-arm precedent generalized. Guest session and owned-device wake become ONE approval flow; lockout = fleet refusing to vouch. Wrapped-root design deliberately deferred to the fleet-key redesign's ira-wrap (don't front-run Nick's review).
