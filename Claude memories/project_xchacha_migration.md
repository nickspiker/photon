...
- **Counter-nonce, unpublished** (call media, new): switch outright, no read-both. `wave/packet.rs` (seq nonce), `wave/spool.rs` (counter). XChaCha buys nothing here (counter never repeats) but uniformity.
