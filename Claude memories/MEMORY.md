...
- [project_self_message_vanish.md](project_self_message_vanish.md) — self/fleet msgs VANISH on restart (is_sibling display rows never persisted, messaging.rs:185 ephemeral gate eats fleet chat not just `$ ` bridge) + never-ACK (phone stale/Pending/unreachable); fix spec'd NOT started (2026-09-02)
...
- [project_device_loaners.md](project_device_loaners.md) — loaners: de-attest keeps claims (dormant); transfer vs loan-annotation UNDECIDED. Removal REVISED 2026-08-31: BILATERAL departures (see self-only-removal); eviction happens at the S/friendship re-key layer; locks/recalls = routing layer
