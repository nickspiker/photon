...
**CUTOVER CLOSED 2026-08-18 (@ 2adff1d):** the ladder is fully dispositioned. Step 4 spine BUILT (see [[project-fleet-epoch-arc-closed]]). Step 3 (§14.5 slot) SUPERSEDED, never built — docs/durability.md decides FLEET-HOLDS-HISTORY: content never touches infrastructure; always-online stays at the coordination minimum; CAN guarantee re-stated over sibling reachability (per-sibling sync cursors + wall-clock W); horizon = per-conversation retention dial; crypto-shred = zeroize per-epoch hist_page seal keys behind a sibling convergence gate; wiped-sole-device = passphrase-sealed export (Phase-later). Step 5 thereby redesigned (build order in durability.md §6, all post-voice-calls except the single-copy UI warning). Step 6 linearizer RETIRED — per-device lanes supersede (lanes residue closed @ b6d4a7a: writer discipline enforced at prepare_send_commit, LANE FORK loud error, label-rotation tripwire test). braid.md §14 header carries the disposition note. NEXT MAJOR: voice calls.

**5ms FLAG DAY 2026-09-08 (calls wire #3, Nick: "flag day it is... batching nuke")**: FRAME_SAMPLES 480→240 end-to-end (platform/Kotlin/engine/preview); TIER_FRAMES [8,4,2,2] (floor keeps 40ms batch, top rungs 10ms windows = 2× packet rate); TIER_MAX_ENC rate/1600+2; PID KI halved/KD doubled/slew 0.35→0.19 (same time-domain response at 2× eval); learner UNTOUCHED — engine+ringback pair 5ms envelopes to its 10ms lattice (StampReg advances one bin per push, 2× cadence would run its clock double); jitter constants re-timed (cap 24, decay 300, ceiling 8) + tier-aware min; recording: PHCALL2 archive stays 10ms (2:1 input fold, old kept blobs play), PHCALL1 READ DELETED (no-compat mandate); Android render buffer 4→3 bursts (underruns= log = monitor); call buttons traffic-light (CALL_ACCEPT/DANGER fills). Structural one-way ≈35-40ms clean LAN.
