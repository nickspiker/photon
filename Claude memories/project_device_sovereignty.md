...
**DELETE SEMANTICS SETTLED 2026-09-08 (docs/vault-delete.md)**: delete = SPACE RECLAMATION on all shipped targets (host CoW + vendor FTLs incl. Pixel-ferros — zero-writes erase nothing there); security boundary = boot-gated vault key (snapshots hold ciphertext); StrongBox rejected; TRUE shred waits for ferros-on-glyph owned NAND. Enforcement = manifestus::DeletePolicy REQUIRED open param (no default — ports must consciously pick UnlinkOnly vs ZeroIsErase). Record-by-default durability: spool register (key‖peer‖offer_osc) persisted at call START, recover_orphans at launch finishes crash-interrupted keeps; ordering blob→register-drop→file-remove.
