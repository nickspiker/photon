...
- Envelope in STOPS below full scale (u8), computed at transcode (frames are decoded there anyway). Thumbnail = one GROSS (144) buckets per channel in the row (siblings draw without the blob); fine envelope 4/s in the container. Buckets-per-pixel derived from card width at zoom, never fixed pixels.
